                ...
                >
                  ...
                  <div class="aspect-[2/3] bg-slate-700 relative overflow-hidden">
                    ...
                    @if (item.rating && item.rating > 0) {
                      <div class="absolute top-2 left-2 flex items-center gap-1 bg-black/70 px-2 py-1 rounded text-xs">
                        ...
                        <span class="text-white font-medium">{{ item.rating.toFixed(1) }}</span>
                      </div>
                    }
                  </div>
                  ...
                  <div class="p-3">
                    ...
                    <div class="mt-1 text-xs text-slate-400">
                      ...
                      @if (getTopGenres(item).length > 0) {
                        <div class="truncate mt-0.5" [title]="getTopGenres(item).join(', ')">
                          {{ getTopGenres(item).join(', ') }}
                        </div>
                      }
                    </div>
                  </div>
...
export class FavoritesRowComponent implements OnInit {
  readonly favoritesService = inject(FavoritesService);
  private readonly router = inject(Router);

  // Signal from the service
  items = this.favoritesService.favoriteItems;

  ngOnInit(): void {
    // Load favorites on component init
    this.favoritesService.getFavorites().subscribe();
  }

  getPosterUrl(item: FavoriteItem): string {
  ...
  getMetadataId(item: FavoriteItem): number {
  ...
  getTopGenres(item: FavoriteItem): string[] {
    return this.favoritesService.getTopGenres(item.genres, 3);
  }

  onCardClick(item: FavoriteItem): void {
    // Navigate to detail page
    if (item.media_type === 'movie' && item.media_id) {
      // Use media_id for navigation
      this.router.navigate(['/movie', item.media_id]);
    } else if (item.media_type === 'series' && item.series_metadata_id) {
