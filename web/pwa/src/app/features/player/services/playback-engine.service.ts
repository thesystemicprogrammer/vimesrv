...
import {
  ...
  FallbackEvent,
  MonitoringMode,
} from '../../../core/services/playback-monitor.service';
...
export class PlaybackEngineService implements OnDestroy {
  ...
  async initialize(
    ...
    if (decision.mode === 'direct' || decision.mode === 'direct-stream') {
      ...
      if (decision.mode === 'direct-stream') {
        ...
        if (audioStreams[audioTrackArrayIndex]) {
          this.selectedAudioStreamIndex =
            audioStreams[audioTrackArrayIndex].index;
        } else {
      ...
      await this.initializeDirectPlayer(
        decision.url,
        decision.mode === 'direct-stream',
      );
      this.playbackModeLabel.set(
        decision.mode === 'direct' ? 'Direct Play' : 'Direct Stream',
      );
    } else {
      // DASH mode
  ...
  continueWatching(seconds: number): void {
    if (!this.player) {
      console.log('Player for continue watching is Null I');
      return;
    }

    this.player.on(dashjs.MediaPlayer.events.STREAM_INITIALIZED, () => {
      if (!this.player) {
        console.log('Player for continue watching is Null II');
        return;
      }
      this.player.seek(seconds);
      this.player.play();

      console.log('Restored watch position:', seconds);
    });
  }
  ...
  seek(seconds: number): void {
    ...
    if (
      this.playbackMode() === 'direct-stream' &&
      newTime < this.videoElement.currentTime
    ) {
      this.config?.onNotification(
        'Seeking backward, switching to adaptive streaming...',
      );
  ...
  private async initializeDirectPlayer(
    url: string,
    appendAudioParam: boolean = false,
  ): Promise<void> {
  ...
  private loadSubtitlesForDirectPlay(): void {
    ...
    this.media.subtitle_streams.forEach((sub, idx) => {
      ...
      track.src = this.streamToken
        ? `${baseUrl}?token=${this.streamToken}`
        : baseUrl;
  ...
  private startPlaybackMonitoring(): void {
    ...
    const monitorMode: MonitoringMode =
      this.playbackMode() === 'direct-stream' ? 'direct-stream' : 'direct';
  ...
  private async fallbackToDash(resumePosition?: number): Promise<void> {
    ...
    console.log(
      'Falling back to DASH streaming',
      resumePosition ? `at ${resumePosition}s` : '',
    );
  ...
  private initializeDashPlayer(
    ...
    try {
      ...
      this.player.on(dashjs.MediaPlayer.events.ERROR, (e: any) => {
        ...
        const errorMsg =
          e.error?.message || e.error?.code || e.event?.type || 'Unknown error';
        const errorDetail = e.error?.data?.response?.status
          ? ` (HTTP ${e.error.data.response.status})`
          : '';
  ...
  private updateQualityLevels(): void {
    ...
    try {
      ...
      const levels: QualityLevel[] = representations.map(
  ...
  private async restartDirectStream(): Promise<void> {
    if (!this.directStreamBaseUrl || !this.streamToken || !this.videoElement)
      return;
    ...
    console.log(
      `Restarting direct stream with audio=${this.selectedAudioStreamIndex} at ${currentTime}s`,
    );
      ...
      if (this.videoElement) {
        ...
        if (wasPlaying) {
          this.videoElement
            .play()
            .catch((err) =>
              console.warn('Autoplay after audio switch blocked:', err),
            );
        }
      }
    };
