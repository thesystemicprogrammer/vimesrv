...
func TestNewDatabaseMigration(t *testing.T) {
	...
	assert.Len(t, dm.migrationJobs, 12)
...
func TestMigrate_FreshDatabase(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 12, count, "all migrations should be applied")
...
func TestMigrate_Idempotency(t *testing.T) {
	...
	err = db.QueryRow("SELECT COUNT(*) FROM schema_migrations").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 12, count, "migrations should not be duplicated on second run")
}
